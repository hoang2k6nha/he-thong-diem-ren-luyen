...
if (!isset($_SESSION['user_id']) || ($_SESSION['role'] !== 'khoa' && !has_permission('department_complaints'))) redirect('index.php');

$khoa_id = $_SESSION['khoa_id'];
$msg = '';
$msg_type = 'success';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'reply') {
    ...
    // Check if complaint belongs to this Khoa or is from a Guest
    $check_sql = "SELECT k.id FROM khieu_nai k LEFT JOIN tai_khoan t ON k.sinh_vien_id = t.id LEFT JOIN lop_hoc l ON t.lop_id = l.id WHERE k.id = $complaint_id AND " . ($_SESSION["role"] !== "khoa" ? "1=1" : "(l.khoa_id = $khoa_id OR k.sinh_vien_id IS NULL)") . "";
...
$complaints = [];
$sql = "SELECT k.*, t.username, t.ho_ten as sv_ten, l.ten_lop 
        FROM khieu_nai k 
        LEFT JOIN tai_khoan t ON k.sinh_vien_id = t.id 
        LEFT JOIN lop_hoc l ON t.lop_id = l.id 
        WHERE k.nguoi_nhan_role = 'khoa' AND " . ($_SESSION["role"] !== "khoa" ? "1=1" : "(l.khoa_id = $khoa_id OR k.sinh_vien_id IS NULL)") . "
...
$page_title = 'Quản Lý Khiếu Nại - KHOA';
            ...
            <div class="complaint-card">
                <div class="c-header">
                    <div>
                        ...
                        <div class="c-meta">
                            <?php if($c['sinh_vien_id']): ?>
                                Sinh viên: <strong><?php echo htmlspecialchars($c['sv_ten']); ?> (<?php echo $c['username']; ?>)</strong> - Lớp: <?php echo $c['ten_lop']; ?> <br>
                            <?php else: ?>
                                Khách: <strong style="color: #D97706;"><?php echo htmlspecialchars($c['ho_ten_khach']); ?> (<?php echo htmlspecialchars($c['mssv_khach']); ?>)</strong> <span class="badge" style="background:#FEF3C7; color:#D97706;">Khách vãng lai</span><br>
                            <?php endif; ?>
